...
import { FEEDS, SNAPSHOTS } from "../src/data/feeds.ts";
...
async function fetchWithTimeout(url) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: { "user-agent": "KhabarDhara-NewsBot/2.0 (+https://github.com/LaxmanNepal/news)" },
    });
...
async function loadSource(src) {
  try {
    const xml = await fetchWithTimeout(src.feed);
    const articles = parseFeed(xml, src);
    if (!articles.length) throw new Error("No articles parsed");
    return { src, articles, ok: true };
  } catch (error) {
    return { src, articles: [], ok: false, error: error instanceof Error ? error.message : String(error) };
  }
}

function categoryFiles(article) {
  const category = FEEDS.find((f) => f.id === article.sourceId)?.category ?? "taja";
  return category;
}
...
for (const result of results) {
  sources.push({
    ...
    site: result.src.site,
    feed: result.src.feed,
    category: result.src.category,
    tag: result.src.tag,
    ok: result.ok,
  ...
  for (const article of result.articles) {
    ...
    seen.add(article.id);
    articles.push({ ...article, category: categoryFiles(article) });
  }
}

articles.sort((a, b) => b.pubDate - a.pubDate);

// Keep a deterministic fallback only when a source is unreachable and has a curated snapshot.
for (const src of FEEDS) {
  const result = results.find((r) => r.src.id === src.id);
  if (!result?.ok && SNAPSHOTS[src.id]) {
    SNAPSHOTS[src.id].forEach((snap, index) => {
      ...
      articles.push({
        ...
        description: snap.d,
        link: src.site,
        pubDate: Date.now() - index * 1800000,
        fromSnapshot: true,
...
const payload = { generatedAt, articleCount: articles.length, sourceCount: FEEDS.length, articles };
...
for (const category of ["rajaniti", "artha", "khelkud", "manoranjan", "prabidhi", "bidesh"]) {
  files[`${category}.json`] = {
    generatedAt,
    articles: articles.filter((a) => a.category === category).slice(0, 100),
  };
}
...
await fs.writeFile(path.join(OUT, "meta.json"), JSON.stringify({
  ...
  successfulSources: results.filter((r) => r.ok).length,
